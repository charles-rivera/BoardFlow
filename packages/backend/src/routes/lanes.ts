...
lanesRouter.get('/', async (req: Request, res: Response): Promise<void> => {
  ...
  const { rows: lanes } = await pool.query(
    'SELECT id, title, position, created_at FROM lanes WHERE user_id = $1 AND deleted_at IS NULL ORDER BY position ASC',
    [userId]
  )
  const { rows: cards } = await pool.query(
    'SELECT id, lane_id, title, description, position, created_at, updated_at FROM cards WHERE user_id = $1 AND deleted_at IS NULL ORDER BY position ASC',
    [userId]
  )
...
lanesRouter.post('/', async (req: Request, res: Response): Promise<void> => {
  ...
  const { rows: [{ max }] } = await pool.query<{ max: number | null }>(
    'SELECT MAX(position) AS max FROM lanes WHERE user_id = $1 AND deleted_at IS NULL',
    [userId]
  )
...
lanesRouter.patch('/:id', async (req: Request, res: Response): Promise<void> => {
  ...
  const { rows, rowCount } = await pool.query(
    'UPDATE lanes SET title = $1 WHERE id = $2 AND user_id = $3 AND deleted_at IS NULL RETURNING id, title, position, created_at',
    [title.trim(), req.params.id, userId]
  )
...
lanesRouter.patch('/:id/reorder', async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const { rowCount } = await client.query(
      'UPDATE lanes SET position = $1 WHERE id = $2 AND user_id = $3 AND deleted_at IS NULL',
      [position, req.params.id, userId]
    )
    if (!rowCount) {
    ...
    const { rows: all } = await client.query<{ id: string; position: number }>(
      'SELECT id, position FROM lanes WHERE user_id = $1 AND deleted_at IS NULL ORDER BY position ASC',
      [userId]
    )
...
lanesRouter.delete('/:id', async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    await client.query('BEGIN')
    const { rowCount } = await client.query(
      'UPDATE lanes SET deleted_at = NOW() WHERE id = $1 AND user_id = $2 AND deleted_at IS NULL RETURNING id',
